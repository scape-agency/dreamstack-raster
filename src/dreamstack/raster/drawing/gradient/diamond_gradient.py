...
def diamond_gradient(
    ...
    *,
    ...
    """Create a diamond gradient image.

    Like radial but uses diamond (taxi-cab) distance.
    ...
    Example:
        >>> grad = diamond_gradient(512, 512, (256, 256), 200)
    """
    # Default stops
    if stops is None:
        stops = [
    ...
    # Diamond distance (L1 / taxi-cab)
    distance = np.abs(x_coords - center[0]) + np.abs(y_coords - center[1])

    # Normalize to [0, 1]
    t = np.clip(distance / max(1, radius), 0, 1).astype(np.float32)

    # Interpolate colors
    return _interpolate_diamond(t, stops)


def _interpolate_diamond(
    t: NDArray[np.float32],
    stops: list[GradientStop],
) -> NDArray[np.uint8]:
    """Interpolate colors for diamond gradient."""
    h, w = t.shape
