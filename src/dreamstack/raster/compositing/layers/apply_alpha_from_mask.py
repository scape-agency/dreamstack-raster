...
"""
Dreamstack Raster - Alpha channel application from mask."""
...
def apply_alpha_from_mask(
    ...
    if image.ndim == 2:
        image = cv2.cvtColor(image, cv2.COLOR_GRAY2BGR)  # type: ignore[assignment]
    ...
    if mask.shape[:2] != image.shape[:2]:
        mask = cv2.resize(mask, (image.shape[1], image.shape[0]))  # type: ignore[assignment]
