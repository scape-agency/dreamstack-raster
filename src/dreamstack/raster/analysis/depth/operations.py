...
"""
Dreamstack Raster - Depth Estimation Operations
===========================
...
def colorize_depth(
    ...
    try:
        ...
        raise ImportError(
            "matplotlib required for colormaps. Install with: pip install matplotlib"
        ) from exc
    ...
    # Apply colormap
    cmap = matplotlib.colormaps.get_cmap(colormap)  # type: ignore
    ...
    *,
    ...
    if colormap is not None:
        ...
        cv2.imwrite(  # pylint: disable=no-member
            str(output_path),
            cv2.cvtColor(
                colored, cv2.COLOR_RGB2BGR
            ),  # pylint: disable=no-member
        )
    elif as_16bit:
        # Save as 16-bit for precision
        depth_16 = (depth * 65535).astype(np.uint16)
        cv2.imwrite(str(output_path), depth_16)  # pylint: disable=no-member
    else:
        # Save as 8-bit grayscale
        depth_8 = (depth * 255).astype(np.uint8)
        cv2.imwrite(str(output_path), depth_8)  # pylint: disable=no-member
...
def save_depth_ply(
    ...
    *,
    ...
    if image.shape[:2] != depth.shape:
        image = np.asarray(
            cv2.resize(
                image, (depth.shape[1], depth.shape[0])
            ),  # pylint: disable=no-member
            dtype=np.uint8,
        )

    # Convert BGR to RGB
    rgb = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)  # pylint: disable=no-member
